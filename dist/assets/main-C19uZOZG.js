import"./style-BzQggXsJ.js";import{createClient as Z}from"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";const Q="https://mkuzksoyetgxjsycfnnx.supabase.co",ee="sb_publishable_seUTej9kixmVRZgvtugmXw__p2MUoU1",h=Z(Q,ee);async function te(){const{data:{user:e}}=await h.auth.getUser();if(e){v=e,console.log("User signed in:",e.email),O.classList.remove("hidden"),loadSavedPortfolios();const{data:t}=await h.from("users").select("*").eq("id",e.id).single();t&&(t.name&&(document.getElementById("name").value=t.name),t.title&&(document.getElementById("role").value=t.title),t.github_url&&(document.getElementById("github").value=t.github_url),m.disabled=!1,m.classList.remove("opacity-50","cursor-not-allowed"))}else localStorage.getItem("guestMode")!=="true"?window.location.href="./login.html":(console.log("Running in Ghost Mode (Guest)"),O.classList.add("hidden"))}te();let j=1,x="developer",w="#00D9FF",D="clash",g=null,v=null,u={};async function oe(){const{data:{user:e}}=await h.auth.getUser();v=e}oe();const P=document.getElementById("portfolio-form"),G=[document.getElementById("step-1"),document.getElementById("step-2"),document.getElementById("step-3"),document.getElementById("step-4")],se=document.querySelectorAll(".next-step"),ae=document.querySelectorAll(".prev-step"),ie=document.querySelectorAll(".step-indicator"),R=document.getElementById("status-container"),N=document.getElementById("preview-empty"),f=document.getElementById("preview-content"),H=document.querySelectorAll("#theme-selector button"),B=document.getElementById("tab-preview"),C=document.getElementById("tab-code"),Y=document.getElementById("preview-frame"),z=document.getElementById("code-editor-pane"),p=document.getElementById("code-editor"),k=document.getElementById("copy-code-btn"),W=document.getElementById("code-info"),V=document.getElementById("preview-label"),O=document.getElementById("my-portfolios-panel");document.getElementById("portfolios-count");document.getElementById("portfolios-list");const m=document.getElementById("save-btn");let U=[];document.querySelectorAll(".color-swatch").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".color-swatch").forEach(t=>t.classList.remove("active")),e.classList.add("active"),w=e.dataset.color,document.documentElement.style.setProperty("--accent",w),g&&T()})});document.querySelectorAll(".font-option").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".font-option").forEach(t=>{t.classList.remove("border-accent/50","bg-accent/10","text-accent"),t.classList.add("border-white/10","bg-white/5")}),e.classList.add("border-accent/50","bg-accent/10","text-accent"),e.classList.remove("border-white/10","bg-white/5"),D=e.dataset.font,g&&T()})});const J=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&(t.target.classList.add("visible"),J.unobserve(t.target))})},{threshold:.1});document.querySelectorAll(".scroll-reveal").forEach(e=>J.observe(e));function le(){B.classList.add("bg-accent","text-black"),B.classList.remove("text-white/40","hover:text-white"),C.classList.remove("bg-accent","text-black"),C.classList.add("text-white/40","hover:text-white"),Y.classList.remove("hidden"),z.classList.add("hidden"),z.classList.remove("flex"),k.classList.add("hidden"),k.classList.remove("flex"),W.classList.add("hidden"),V.classList.remove("hidden")}function ne(){C.classList.add("bg-accent","text-black"),C.classList.remove("text-white/40","hover:text-white"),B.classList.remove("bg-accent","text-black"),B.classList.add("text-white/40","hover:text-white"),Y.classList.add("hidden"),z.classList.remove("hidden"),z.classList.add("flex"),k.classList.remove("hidden"),k.classList.add("flex"),W.classList.remove("hidden"),V.classList.add("hidden"),g&&!p.dataset.userEdited&&(p.value=f.innerHTML)}B.addEventListener("click",le);C.addEventListener("click",ne);p.addEventListener("input",()=>{p.dataset.userEdited="true",f.innerHTML=p.value,f.classList.remove("opacity-20","pointer-events-none","select-none"),N.classList.add("hidden")});k.addEventListener("click",async()=>{const e=p.value||f.innerHTML;if(e)try{await navigator.clipboard.writeText(e);const t=k.querySelector("span");t.textContent="Copied!",k.classList.add("border-accent/50","text-accent"),setTimeout(()=>{t.textContent="Copy",k.classList.remove("border-accent/50","text-accent")},2e3)}catch(t){alert("Could not copy: "+t.message)}});window.addEventListener("beforeunload",e=>{if(p.dataset.userEdited==="true"&&g){const t="You have unsaved changes to your portfolio code. Are you sure you want to leave?";return e.returnValue=t,t}});function K(){G.forEach((e,t)=>{t+1===j?(e.classList.remove("hidden"),e.classList.add("animate-fade-in")):e.classList.add("hidden")}),ie.forEach((e,t)=>{t+1<=j?(e.classList.remove("bg-white/10"),e.classList.add("bg-accent")):(e.classList.add("bg-white/10"),e.classList.remove("bg-accent"))})}se.forEach(e=>{e.addEventListener("click",()=>{j<G.length&&(j++,K())})});ae.forEach(e=>{e.addEventListener("click",()=>{j>1&&(j--,K())})});P.addEventListener("submit",async e=>{var l,a,d;e.preventDefault();const t={name:document.getElementById("name").value,role:document.getElementById("role").value,taglineInput:((l=document.getElementById("tagline-input"))==null?void 0:l.value)||"",skills:document.getElementById("skills").value,projects:document.getElementById("projects").value,github:document.getElementById("github").value,email:((a=document.getElementById("email"))==null?void 0:a.value)||"",linkedin:((d=document.getElementById("linkedin"))==null?void 0:d.value)||"",accentColor:w,font:D};u=t,R.classList.remove("hidden"),P.classList.add("opacity-50","pointer-events-none");try{const{data:i,error:b}=await h.functions.invoke("generate-portfolio",{body:t});b?(console.warn("Edge Function failed or key missing. Falling back to Demo Mode."),g=q(t)):g=i,T(),re(t,g),document.getElementById("download-btn").disabled=!1,document.getElementById("download-btn").classList.remove("opacity-40","cursor-not-allowed"),document.getElementById("readme-btn").disabled=!1,document.getElementById("readme-btn").classList.remove("opacity-40","cursor-not-allowed"),document.getElementById("share-btn").disabled=!1,document.getElementById("share-btn").classList.remove("opacity-40","cursor-not-allowed"),v&&(m.disabled=!1,m.classList.remove("opacity-40","cursor-not-allowed"))}catch{console.warn("AI Generation trigger failed. Falling back to Demo Mode."),g=q(t),T()}finally{R.classList.add("hidden"),P.classList.remove("opacity-50","pointer-events-none")}});async function re(e,t){const{data:{user:l}}=await h.auth.getUser();if(!l){console.log("No authenticated user — skipping database save (guest mode).");return}try{console.log("Saving portfolio to Supabase for user:",l.id);const a=l.id,d=e.name.toLowerCase().replace(/\s+/g,".").replace(/[^a-z0-9.]/g,"")+"_"+a.slice(0,6),{error:i}=await h.from("users").upsert({id:a,username:d,name:e.name,title:e.role,bio:t.about,github_url:e.github||null,selected_template:x},{onConflict:"id"});if(i){console.error("User upsert error:",i.message);return}console.log("✅ User profile saved.")}catch(a){console.error("Failed to save portfolio to database:",a.message)}}function q(e){const t=e.skills.split(",").map(a=>a.trim()).filter(Boolean),l=(e.projects||"Awesome Project, NextGen App").split(",").map(a=>a.trim()).filter(Boolean);return{tagline:`Crafting high-performance ${e.role} experiences with precision & creative flair.`,about:`I'm a passionate ${e.role} who transforms complex challenges into elegant, user-centric solutions. With deep expertise in ${t.slice(0,3).join(", ")}, I build scalable products that people love. I care deeply about clean code, great design, and shipping fast.`,skills:t,github:e.github||"",projects:l.map((a,d)=>({title:a,description:"A modern, production-grade platform built to deliver an exceptional user experience. Focused on performance, accessibility, and beautiful design.",tech:t.slice(0,3),color:["#00D9FF","#6C63FF","#FF6584","#43D9AD"][d%4]}))}}function M(e){const t={html:"🌐",css:"🎨",javascript:"⚡",js:"⚡",typescript:"📘",ts:"📘",react:"⚛️",vue:"💚",angular:"🔴",node:"🟢",nodejs:"🟢",python:"🐍",java:"☕",php:"🐘",go:"🐹",rust:"🦀",swift:"🍎",kotlin:"🟣",flutter:"💙",dart:"🎯",supabase:"⚡",firebase:"🔥",aws:"☁️",docker:"🐳",git:"📦",figma:"🎭",tailwind:"💨",next:"▲",nextjs:"▲",mongodb:"🍃",sql:"🗄️",mysql:"🗄️",postgres:"🐘",postgresql:"🐘",graphql:"🔷",redux:"🟣",express:"🚂",django:"🎸",rails:"💎",laravel:"🔴"},l=e.toLowerCase().replace(/[^a-z]/g,"");return t[l]||"🔧"}function ce(){typeof gsap>"u"||(gsap.fromTo("#preview-content .portfolio-hero",{opacity:0,y:40},{opacity:1,y:0,duration:.7,ease:"power3.out"}),gsap.fromTo("#preview-content .portfolio-section",{opacity:0,y:30},{opacity:1,y:0,duration:.6,ease:"power2.out",stagger:.15,delay:.2}),document.querySelectorAll(".skill-bar-fill").forEach(e=>{const t=e.dataset.width||"80%";gsap.fromTo(e,{width:"0%"},{width:t,duration:1,ease:"power2.out",delay:.5})}))}H.forEach(e=>{e.addEventListener("click",()=>{if(console.log("Theme button clicked:",e.dataset.theme),H.forEach(t=>{t.classList.remove("bg-accent","text-black"),t.classList.add("hover:bg-white/5")}),e.classList.add("bg-accent","text-black"),e.classList.remove("hover:bg-white/5"),x=e.dataset.theme,g)T(),v&&document.getElementById("name").value&&h.from("users").update({selected_template:x}).match({id:v.id}).then(()=>{console.log("Theme saved to DB")});else{const t=document.querySelector("#preview-empty h3");t&&(t.innerHTML=`AI Portfolio Ready <span class="text-accent text-sm block mt-2 font-mono">Theme: ${x}</span>`)}console.log("Current theme set to:",x)})});function T(){if(!g)return;N.classList.add("hidden"),f.classList.remove("hidden","opacity-20","pointer-events-none"),f.innerHTML="",f.classList.add("select-text");const{tagline:e,about:t,skills:l,projects:a,github:d}=g,i=document.getElementById("name").value||"Your Name",b=document.getElementById("role").value||"Developer",c=document.getElementById("github").value||d||"",$=i.split(" ").map(o=>o[0]).join("").toUpperCase().slice(0,2),I="./images/hero.svg";function _(o){return o.split("").reduce((r,n)=>r+n.charCodeAt(0),42)%29+70+"%"}const S={clash:'font-family: "Clash Display", system-ui, sans-serif !important;',inter:'font-family: "Inter", system-ui, sans-serif !important;',mono:"font-family: monospace !important;"},L=S[D]||S.clash;let y="";if(x==="developer"?y=`
        <div class="portfolio-root font-mono text-white bg-[#070710] min-h-screen" style="--accent: ${w}; ${L}">
            <!-- HERO -->
            <div class="portfolio-hero relative overflow-hidden">
                <!-- Background image with overlay -->
                <div class="absolute inset-0">
                    <img src="${I}" class="w-full h-full object-cover opacity-10" alt="hero">
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex gap-2 flex-wrap">
                                        ${o.tech.map(n=>`<span class="text-[10px] bg-white/5 border border-white/10 px-3 py-1 rounded-full text-white/50 hover:border-accent/40 hover:text-accent transition-all">${n}</span>`).join("")}
                                    </div>
        ...
        <div class="portfolio-root font-[system-ui] bg-white text-[#111] min-h-screen -m-12 pb-0" style="--accent: ${w}; ${L}">
            <!-- HERO -->
            <div class="portfolio-hero relative overflow-hidden bg-white pt-20 pb-0 px-12">
                ...
                <div class="relative h-[360px] overflow-hidden rounded-t-[40px] mx-auto max-w-5xl shadow-[0_-20px_60px_rgba(0,0,0,0.08)] bg-slate-100">
                    <img src="${I}" class="w-full h-full object-cover" alt="${i}">
                </div>
            </div>
                        ...
                        <div class="group">
                            ...
                            <div class="flex items-start justify-between gap-8">
                                ...
                                <div class="flex flex-col gap-2 flex-shrink-0">
                                    ${o.tech.map(n=>`<span class="text-[10px] bg-slate-100 px-3 py-1.5 rounded-lg font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">${n}</span>`).join("")}
                                </div>
                            </div>
                        </div>`).join("")}
                </div>
        ...
        <div class="portfolio-root text-white min-h-screen pb-20" style="background:linear-gradient(135deg,#06060F 0%,#0E0720 50%,#06060F 100%); --accent: ${w}; ${L}">
            <!-- Ambient blobs -->
            ...
            <div class="grid grid-cols-3 gap-4 px-10 py-8 h-[400px]">
                <div class="col-span-2 rounded-[30px] overflow-hidden relative bg-black/50">
                    <img src="${I}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-1000" alt="${i}">
            ...
            <div class="portfolio-section px-10 pb-16">
                ...
                <div class="flex flex-wrap gap-3">
                    ${l.map((o,s)=>{const r=["rgba(0,217,255,0.15)","rgba(139,92,246,0.15)","rgba(236,72,153,0.15)","rgba(34,197,94,0.15)"],n=["rgba(0,217,255,0.3)","rgba(139,92,246,0.3)","rgba(236,72,153,0.3)","rgba(34,197,94,0.3)"],E=r[s%4],A=n[s%4];return`<span class="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium hover:scale-105 transition-transform cursor-default" style="background:${E};border:1px solid ${A}">
                            ${M(o)} ${o}
                        </span>`}).join("")}
                </div>
            </div>
                        ...
                        <div class="group relative overflow-hidden rounded-[30px] border border-white/5 hover:border-white/15 transition-all duration-700" style="background:rgba(255,255,255,0.025)">
                            <div class="grid grid-cols-5 gap-0 h-56">
                                ...
                                <div class="col-span-2 p-7 flex flex-col justify-between">
                                    ...
                                    <div class="flex gap-2 flex-wrap">
                                        ${o.tech.map(n=>`<span class="text-[10px] px-3 py-1 rounded-full border border-white/10 text-white/50">${n}</span>`).join("")}
                                    </div>
                                </div>
                            </div>
                        </div>`).join("")}
        ...
        </div>`),x==="minimal"){const o=w;y=`
        <div class="portfolio-root" style="background:#fff;color:#111;min-height:100vh;${L}">
            <!-- Header -->
            <div class="portfolio-hero" style="max-width:680px;margin:0 auto;padding:72px 24px 48px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:48px">
                    ...
                    ${c?`<a href="https://github.com/${c}" target="_blank" style="color:#888;font-size:13px;text-decoration:none;border:1px solid #eee;padding:6px 14px;border-radius:20px;transition:all 0.2s" onmouseover="this.style.borderColor='${o}'" onmouseout="this.style.borderColor='#eee'">GitHub ↗</a>`:""}
                </div>
                ...
                <div style="display:flex;flex-direction:column;gap:24px">
                    ...
                    <div style="padding:24px;border:1px solid #eee;border-radius:16px;transition:all 0.2s" class="card-hover">
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
                            ...
                            <span style="font-size:0.7rem;color:#aaa;font-family:monospace">0${r+1}</span>
                        </div>
                        <p style="font-size:0.875rem;color:#666;line-height:1.6;margin-bottom:12px">${s.description}</p>
                        <div style="display:flex;gap:6px;flex-wrap:wrap">
                            ${s.tech.map(n=>`<span style="font-size:0.7rem;padding:2px 8px;background:#f5f5f5;color:#666;border-radius:10px">${n}</span>`).join("")}
                        </div>
                    </div>`).join("")}
                </div>
            </div>
            ...
            <div style="border-top:1px solid #eee;max-width:680px;margin:0 auto;padding:32px 24px;display:flex;justify-content:space-between;align-items:center">
                ...
                ${u!=null&&u.email?`<a href="mailto:${u.email}" style="font-size:0.85rem;color:${o};text-decoration:none;font-weight:600">${u.email}</a>`:""}
            </div>
        </div>`}if(x==="bold"){const o=w;y=`
        <div class="portfolio-root" style="background:#000;color:#fff;min-height:100vh;${L}">
            <!-- HERO BANNER -->
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;min-height:400px">
                ...
                <div class="portfolio-section" style="padding:48px;border-right:1px solid #222">
                    ...
                    ${c?`<a href="https://github.com/${c}" target="_blank" style="margin-top:24px;display:inline-flex;align-items:center;gap:8px;color:${o};font-size:0.8rem;font-weight:700;text-decoration:none">View GitHub ↗</a>`:""}
                </div>
                <!-- Skills -->
            ...
            <div class="portfolio-section" style="padding:48px;border-top:1px solid #222">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:2px">
                    ${a.map((s,r)=>{const n=[o,"#1a1a1a","#111","#0a0a0a"],E=r===0?"#000":"#fff";return`
                    <div class="card-hover" style="background:${n[r%n.length]};padding:40px 32px;cursor:default">
                        <div style="font-size:2rem;font-weight:900;color:${r===0?"rgba(0,0,0,0.15)":"rgba(255,255,255,0.06)"};font-family:monospace;margin-bottom:24px">0${r+1}</div>
                        <h3 style="font-size:1.4rem;font-weight:900;color:${E};letter-spacing:-1px;margin-bottom:12px">${s.title}</h3>
                        <p style="font-size:0.8rem;color:${r===0?"rgba(0,0,0,0.65)":"#666"};line-height:1.6;margin-bottom:20px;font-family:'Inter',sans-serif">${s.description}</p>
                        <div style="display:flex;gap:6px;flex-wrap:wrap">
                            ${s.tech.map(A=>`<span style="font-size:0.65rem;padding:3px 10px;background:${r===0?"rgba(0,0,0,0.15)":"rgba(255,255,255,0.08)"};border-radius:20px;color:${r===0?"rgba(0,0,0,0.7)":"#888"};font-weight:700">${A}</span>`).join("")}
                        </div>
                    </div>`}).join("")}
                </div>
            </div>
...
<body>${e}</body>
</html>`,a=new Blob([l],{type:"text/html"}),d=URL.createObjectURL(a),i=document.createElement("a");i.href=d,i.download=`${t.toLowerCase().replace(/\s/g,"-")}-portfolio.html`,i.click(),URL.revokeObjectURL(d)});document.getElementById("share-btn").addEventListener("click",async()=>{let e=(document.getElementById("name").value||"user").toLowerCase().replace(/\s/g,"-")+"-"+Math.random().toString(36).substring(2,6);if(v){const{data:t}=await h.from("users").select("username").match({id:v.id}).single();t&&t.username&&(e=t.username),await h.from("users").update({is_published:!0}).match({id:v.id})}alert(`🚀 Your portfolio is live at:
https://portfolio.ai/${e}

(Copy this link to share with anyone!)`)});document.getElementById("readme-btn").addEventListener("click",()=>{var s,r;if(!g)return;const e=document.getElementById("name").value||"Developer",t=document.getElementById("role").value||"Developer",l=document.getElementById("github").value||"",a=((s=document.getElementById("linkedin"))==null?void 0:s.value)||"",d=((r=document.getElementById("email"))==null?void 0:r.value)||"",{tagline:i,about:b,skills:c,projects:$}=g,I=c.map(n=>`![${n}](https://img.shields.io/badge/${encodeURIComponent(n)}-informational?style=flat&color=00D9FF)`).join(" "),_=$.map((n,E)=>`### ${E+1}. ${n.title}
${n.description}

**Tech:** ${n.tech.join(", ")}`).join(`

---

`),S=[l?`[🐙 GitHub](https://github.com/${l})`:null,a?`[💼 LinkedIn](https://${a.replace("https://","")})`:null,d?`[📧 Email](mailto:${d})`:null].filter(Boolean).join(" · "),L=`<div align="center">
...
`,y=new Blob([L],{type:"text/markdown"}),F=URL.createObjectURL(y),o=document.createElement("a");o.href=F,o.download="README.md",o.click(),URL.revokeObjectURL(F)});
