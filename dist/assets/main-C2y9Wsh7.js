import"./style-B6nEs98U.js";import{createClient as F}from"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";const D="https://mkuzksoyetgxjsycfnnx.supabase.co",P="sb_publishable_seUTej9kixmVRZgvtugmXw__p2MUoU1",c=F(D,P);async function M(){const{data:{session:e}}=await c.auth.getSession(),t=localStorage.getItem("guest_mode")==="true";!e&&!t&&(window.location.href="login.html")}M();let y=1,x="developer",v=null,w=null;async function U(){const{data:{user:e}}=await c.auth.getUser();w=e}U();const $=document.getElementById("dark-mode-toggle");$.addEventListener("click",()=>{const e=document.documentElement.classList.toggle("dark");localStorage.theme=e?"dark":"light",$.style.transform="scale(0.85) rotate(20deg)",setTimeout(()=>{$.style.transform=""},200)});const j=document.getElementById("portfolio-form"),S=[document.getElementById("step-1"),document.getElementById("step-2")],O=document.querySelectorAll(".next-step"),R=document.querySelectorAll(".prev-step"),z=document.querySelectorAll(".step-indicator"),B=document.getElementById("status-container"),H=document.getElementById("preview-empty"),p=document.getElementById("preview-content"),_=document.querySelectorAll("#theme-selector button");function A(){S.forEach((e,t)=>{t+1===y?(e.classList.remove("hidden"),e.classList.add("animate-fade-in")):e.classList.add("hidden")}),z.forEach((e,t)=>{t+1<=y?(e.classList.remove("bg-white/10"),e.classList.add("bg-accent")):(e.classList.add("bg-white/10"),e.classList.remove("bg-accent"))})}O.forEach(e=>{e.addEventListener("click",()=>{y<S.length&&(y++,A())})});R.forEach(e=>{e.addEventListener("click",()=>{y>1&&(y--,A())})});j.addEventListener("submit",async e=>{e.preventDefault();const t={name:document.getElementById("name").value,role:document.getElementById("role").value,skills:document.getElementById("skills").value,projects:document.getElementById("projects").value,github:document.getElementById("github").value};B.classList.remove("hidden"),j.classList.add("opacity-50","pointer-events-none");try{const{data:l,error:a}=await c.functions.invoke("generate-portfolio",{body:t});a?(console.warn("Edge Function failed or key missing. Falling back to Demo Mode."),v=C(t)):v=l,L(),q(t,v),document.getElementById("download-btn").disabled=!1,document.getElementById("download-btn").classList.remove("opacity-50","cursor-not-allowed"),document.getElementById("share-btn").disabled=!1,document.getElementById("share-btn").classList.remove("opacity-50","cursor-not-allowed")}catch{console.warn("AI Generation trigger failed. Falling back to Demo Mode."),v=C(t),L()}finally{B.classList.add("hidden"),j.classList.remove("opacity-50","pointer-events-none")}});async function q(e,t){const{data:{user:l}}=await c.auth.getUser();if(!l){console.log("No authenticated user — skipping database save (guest mode).");return}try{console.log("Saving portfolio to Supabase for user:",l.id);const a=l.id,g=e.name.toLowerCase().replace(/\s+/g,".").replace(/[^a-z0-9.]/g,"")+"_"+a.slice(0,6),{error:o}=await c.from("users").upsert({id:a,username:g,name:e.name,title:e.role,bio:t.about,github_url:e.github||null,selected_template:x},{onConflict:"id"});if(o){console.error("User upsert error:",o.message);return}console.log("✅ User profile saved.");const{error:b}=await c.from("skills").delete().eq("user_id",a);if(b){console.error("Skills delete error:",b.message);return}if(t.skills&&t.skills.length>0){const h=t.skills.map((f,m)=>({user_id:a,skill_name:f,category:"General",position:m})),{error:d}=await c.from("skills").insert(h);if(d){console.error("Skills insert error:",d.message);return}console.log("✅ Skills saved:",t.skills.length)}const{error:r}=await c.from("projects").delete().eq("user_id",a);if(r){console.error("Projects delete error:",r.message);return}if(t.projects&&t.projects.length>0){const h=t.projects.map((f,m)=>({user_id:a,title:f.title,description:f.description,tech_stack:f.tech,position:m})),{error:d}=await c.from("projects").insert(h);if(d){console.error("Projects insert error:",d.message);return}console.log("✅ Projects saved:",t.projects.length)}console.log("🎉 Portfolio fully saved to Supabase!")}catch(a){console.error("Failed to save portfolio to database:",a.message)}}function C(e){const t=e.skills.split(",").map(a=>a.trim()).filter(Boolean),l=(e.projects||"Awesome Project, NextGen App").split(",").map(a=>a.trim()).filter(Boolean);return{tagline:`Crafting high-performance ${e.role} experiences with precision & creative flair.`,about:`I'm a passionate ${e.role} who transforms complex challenges into elegant, user-centric solutions. With deep expertise in ${t.slice(0,3).join(", ")}, I build scalable products that people love. I care deeply about clean code, great design, and shipping fast.`,skills:t,github:e.github||"",projects:l.map((a,g)=>({title:a,description:"A modern, production-grade platform built to deliver an exceptional user experience. Focused on performance, accessibility, and beautiful design.",tech:t.slice(0,3),color:["#00D9FF","#6C63FF","#FF6584","#43D9AD"][g%4]}))}}function E(e){const t={html:"🌐",css:"🎨",javascript:"⚡",js:"⚡",typescript:"📘",ts:"📘",react:"⚛️",vue:"💚",angular:"🔴",node:"🟢",nodejs:"🟢",python:"🐍",java:"☕",php:"🐘",go:"🐹",rust:"🦀",swift:"🍎",kotlin:"🟣",flutter:"💙",dart:"🎯",supabase:"⚡",firebase:"🔥",aws:"☁️",docker:"🐳",git:"📦",figma:"🎭",tailwind:"💨",next:"▲",nextjs:"▲",mongodb:"🍃",sql:"🗄️",mysql:"🗄️",postgres:"🐘",postgresql:"🐘",graphql:"🔷",redux:"🟣",express:"🚂",django:"🎸",rails:"💎",laravel:"🔴"},l=e.toLowerCase().replace(/[^a-z]/g,"");return t[l]||"🔧"}function G(){typeof gsap>"u"||(gsap.fromTo("#preview-content .portfolio-hero",{opacity:0,y:40},{opacity:1,y:0,duration:.7,ease:"power3.out"}),gsap.fromTo("#preview-content .portfolio-section",{opacity:0,y:30},{opacity:1,y:0,duration:.6,ease:"power2.out",stagger:.15,delay:.2}),document.querySelectorAll(".skill-bar-fill").forEach(e=>{const t=e.dataset.width||"80%";gsap.fromTo(e,{width:"0%"},{width:t,duration:1,ease:"power2.out",delay:.5})}))}_.forEach(e=>{e.addEventListener("click",()=>{if(console.log("Theme button clicked:",e.dataset.theme),_.forEach(t=>{t.classList.remove("bg-accent","text-black"),t.classList.add("hover:bg-white/5")}),e.classList.add("bg-accent","text-black"),e.classList.remove("hover:bg-white/5"),x=e.dataset.theme,v)L(),w&&document.getElementById("name").value&&c.from("users").update({selected_template:x}).match({id:w.id}).then(()=>{console.log("Theme saved to DB")});else{const t=document.querySelector("#preview-empty h3");t&&(t.innerHTML=`AI Portfolio Ready <span class="text-accent text-sm block mt-2 font-mono">Theme: ${x}</span>`)}console.log("Current theme set to:",x)})});function L(){if(!v)return;H.classList.add("hidden"),p.classList.remove("hidden","opacity-20","pointer-events-none"),p.innerHTML="",p.classList.add("select-text");const{tagline:e,about:t,skills:l,projects:a,github:g}=v,o=document.getElementById("name").value||"Your Name",b=document.getElementById("role").value||"Developer",r=document.getElementById("github").value||g||"",h=o.split(" ").map(s=>s[0]).join("").toUpperCase().slice(0,2),d="./images/hero.svg";function f(s){return s.split("").reduce((u,n)=>u+n.charCodeAt(0),42)%29+70+"%"}let m="";x==="developer"?m=`
        <div class="portfolio-root font-mono text-white bg-[#070710] min-h-screen">
            ...
            <div class="portfolio-hero relative overflow-hidden">
                ...
                <div class="absolute inset-0">
                    <img src="${d}" class="w-full h-full object-cover opacity-10" alt="hero">
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex gap-2 flex-wrap">
                                        ${s.tech.map(n=>`<span class="text-[10px] bg-white/5 border border-white/10 px-3 py-1 rounded-full text-white/50 hover:border-accent/40 hover:text-accent transition-all">${n}</span>`).join("")}
                                    </div>
        ...
        <div class="portfolio-root font-[system-ui] bg-white text-[#111] min-h-screen -m-12 pb-0">
            ...
            <div class="portfolio-hero relative overflow-hidden bg-white pt-20 pb-0 px-12">
                ...
                <div class="relative h-[360px] overflow-hidden rounded-t-[40px] mx-auto max-w-5xl shadow-[0_-20px_60px_rgba(0,0,0,0.08)] bg-slate-100">
                    <img src="${d}" class="w-full h-full object-cover" alt="${o}">
                </div>
            </div>
                        ...
                        <div class="group">
                            ...
                            <div class="flex items-start justify-between gap-8">
                                ...
                                <div class="flex flex-col gap-2 flex-shrink-0">
                                    ${s.tech.map(n=>`<span class="text-[10px] bg-slate-100 px-3 py-1.5 rounded-lg font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">${n}</span>`).join("")}
                                </div>
                            </div>
                        </div>`).join("")}
                </div>
        ...
        <div class="portfolio-root text-white min-h-screen pb-20" style="background:linear-gradient(135deg,#06060F 0%,#0E0720 50%,#06060F 100%)">
            ...
            <div class="grid grid-cols-3 gap-4 px-10 py-8 h-[400px]">
                <div class="col-span-2 rounded-[30px] overflow-hidden relative bg-black/50">
                    <img src="${d}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-1000" alt="${o}">
            ...
            <div class="portfolio-section px-10 pb-16">
                ...
                <div class="flex flex-wrap gap-3">
                    ${l.map((s,i)=>{const u=["rgba(0,217,255,0.15)","rgba(139,92,246,0.15)","rgba(236,72,153,0.15)","rgba(34,197,94,0.15)"],n=["rgba(0,217,255,0.3)","rgba(139,92,246,0.3)","rgba(236,72,153,0.3)","rgba(34,197,94,0.3)"],k=u[i%4],I=n[i%4];return`<span class="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium hover:scale-105 transition-transform cursor-default" style="background:${k};border:1px solid ${I}">
                            ${E(s)} ${s}
                        </span>`}).join("")}
                </div>
            </div>
                        ...
                        <div class="group relative overflow-hidden rounded-[30px] border border-white/5 hover:border-white/15 transition-all duration-700" style="background:rgba(255,255,255,0.025)">
                            <div class="grid grid-cols-5 gap-0 h-56">
                                ...
                                <div class="col-span-2 p-7 flex flex-col justify-between">
                                    ...
                                    <div class="flex gap-2 flex-wrap">
                                        ${s.tech.map(n=>`<span class="text-[10px] px-3 py-1 rounded-full border border-white/10 text-white/50">${n}</span>`).join("")}
                                    </div>
                                </div>
                            </div>
                        </div>`).join("")}
        ...
        </div>`),p.innerHTML=m,requestAnimationFrame(G),p.querySelectorAll('a[href^="#"]').forEach(s=>{s.addEventListener("click",function(i){i.preventDefault();const u=this.getAttribute("href").substring(1),n=p.querySelector("#"+u);if(n){const k=p.getBoundingClientRect().top,T=n.getBoundingClientRect().top-k;p.scrollBy({top:T,behavior:"smooth"})}})})}document.getElementById("download-btn").addEventListener("click",()=>{if(!v)return;const e=p.innerHTML,t=document.getElementById("name").value||"portfolio",l=`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>${e}</body>
</html>`,a=new Blob([l],{type:"text/html"}),g=URL.createObjectURL(a),o=document.createElement("a");o.href=g,o.download=`${t.toLowerCase().replace(/\s/g,"-")}-portfolio.html`,o.click(),URL.revokeObjectURL(g)});document.getElementById("share-btn").addEventListener("click",async()=>{let e=(document.getElementById("name").value||"user").toLowerCase().replace(/\s/g,"-")+"-"+Math.random().toString(36).substring(2,6);if(w){const{data:t}=await c.from("users").select("username").match({id:w.id}).single();t&&t.username&&(e=t.username),await c.from("users").update({is_published:!0}).match({id:w.id})}alert(`🚀 Your portfolio is live at:
